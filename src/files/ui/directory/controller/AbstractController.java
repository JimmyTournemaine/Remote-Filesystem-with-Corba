...
class AbstractController implements ActionListener {
    ...
    protected void openDirectory() {
        ...
        try {
            model.getCurrent().open_directory(dirHolder, entry.name);
            model.setCurrent(dirHolder.value);
    ...
    protected void openFile() {
        ...
        if (entry == null) return;
        try {
            new RegularFileDialog(model.getCurrent(), entry.name);
        } catch (no_such_file e1) {
            showErrorMessage(e1, entry.name + " does not exist.");
        } catch (invalid_type_file e1) {
            showErrorMessage(e1, entry.name + " is not a regular file.");
    ...
    protected void newDirectory() {
        ...
        try {
            model.getCurrent().create_directory(new directoryHolder(), name);
            model.refresh();
    ...
    protected void newFile() {
        ...
        if (name == null) return;
        try {
            model.getCurrent().create_regular_file(new regular_fileHolder(), name);
            model.refresh();
        } catch (already_exist e1) {
            JOptionPane.showMessageDialog(view, "The name " + name + " is already used.");
        } catch (io e1) {
    ...
    protected void delete() {
        directory_entry entry = (directory_entry) view.getSelectedValue();
        if (entry == null || entry == model.firstElement()) // first is parent
            return;
        try {
            if (JOptionPane.showConfirmDialog(view, "Are you sure to delete " + entry.name + " ?",
                    null, JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
                model.getCurrent().delete_file(entry.name);
                model.refresh();
            }
        } catch (Exception e1) {
            e1.printStackTrace();
